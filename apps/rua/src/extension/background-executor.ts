...
export function createMainContextRuaAPI(
  ...
  return {
    ...
    on(
      ...
      callback: (() => void) | ((data: ActionTriggeredData | SearchChangeData) => void)
    ): void {
      if (event === "activate") {
...
export async function executeBackgroundScript(
  ...
  console.log(
    "[BackgroundExecutor] Loading background script for:",
    extensionId,
    "url:",
    importUrl
  );
  ...
  try {
    ...
    if (errorMessage === "Script initialization timeout") {
      ...
      console.error(
        "[BackgroundExecutor] Failed to load background script for:",
        extensionId,
        error
      );
    }
...
export async function notifyActionTriggered(
  ...
  if (!state?.loaded) {
    console.warn(
      "[BackgroundExecutor] Cannot notify action-triggered: extension not loaded:",
      extensionId
    );
    return;
  }
    ...
    promises.push(
      (async () => {
        try {
          ...
          console.warn(
            "[BackgroundExecutor] Failed to notify action-triggered:",
            extensionId,
            error
          );
        }
      ...
      promises.push(
        (async () => {
          try {
            ...
            console.warn(
              "[BackgroundExecutor] Failed to notify search-change:",
              extensionId,
              error
            );
          }
