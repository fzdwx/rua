...
function parseInput(input: string): ParseResult | null {
  ...
  const timezonePatterns = [/^(.+?)\s+(?:to|in)\s+([A-Za-z0-9_+\-/:]+)$/i];
...
function getSmartHints(
  ...
  if (trimmed === "" || trimmed === "n" || trimmed === "no" || trimmed === "now") {
    hints.push({
      category: "内置函数",
      hints: [{ code: "now()", description: "显示当前时间（实时更新）" }],
    });
  }
  ...
  if (
    ...
    (trimmed.includes("+") || trimmed.includes("-") || trimmed.match(/^now\s*$/))
  ) {
  ...
  if (
    (trimmed.includes(" to ") ||
      ...
      trimmed.endsWith(" to") ||
      trimmed.endsWith(" in") ||
      trimmed.match(/\s+to\s+\w*$/i) ||
      trimmed.match(/\s+in\s+\w*$/i) ||
      trimmed.match(/\s+t$/i) ||
      trimmed.match(/\s+i$/i)) &&
