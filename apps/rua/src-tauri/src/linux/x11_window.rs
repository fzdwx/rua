use x11rb::{
  connection::Connection, protocol::xproto::*, rust_connection::RustConnection, CURRENT_TIME,
};
...
impl X11WindowManager {
  ...
  pub fn new() -> Option<Self> {
    match RustConnection::connect(None) {
      Ok((conn, screen_num)) => {
        eprintln!(
          "[X11] Connected to X11 display server (screen: {})",
          screen_num
        );
    ...
    for window in windows {
      if let Some((instance, class)) = get_window_class(&self.conn, window) {
        if instance == class_name || class == class_name {
          eprintln!(
            "[X11] Found window: {} (instance={}, class={})",
  ...
  pub fn hide_window(&self, window: Window) -> Result<(), String> {
    ...
    self
      .conn
      .unmap_window(window)
      .map_err(|e| format!("unmap_window failed: {}", e))?;
