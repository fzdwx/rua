...
pub async fn get_extensions(app: AppHandle) -> Result<Vec<ExtensionInfo>, String> {
  ...
  let entries =
    fs::read_dir(&extensions_dir).map_err(|e| format!("Failed to read extensions dir: {}", e))?;
...
pub async fn install_extension(
  ...
  if source_path.starts_with("github:") {
    ...
    // Find .rua asset
    let rua_asset = release
      .assets
      .iter()
      .find(|a| a.name.ends_with(".rua"))
      .ok_or(format!(
        "No .rua file found in release {}",
        release.tag_name
      ))?;
...
pub async fn uninstall_extension(
  ...
  if ext_path.exists() {
    fs::remove_dir_all(&ext_path).map_err(|e| format!("Failed to remove extension: {}", e))?;
  }
