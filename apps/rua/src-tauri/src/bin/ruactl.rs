...
fn load_ignore_patterns(dir: &Path) -> Vec<glob::Pattern> {
  ...
  if ruaignore_path.exists() {
    if let Ok(file) = File::open(&ruaignore_path) {
      ...
      patterns_str = DEFAULT_IGNORE_PATTERNS
        .iter()
        .map(|s| s.to_string())
        .collect();
    }
...
fn get_extensions_dir() -> Result<PathBuf, String> {
  ...
  let extensions_dir = PathBuf::from(home).join(".local/share/like.rua.ai/extensions");
...
fn extract_rua_archive(archive_data: &[u8], extensions_dir: &Path) -> Result<String, String> {
  ...
  let mut archive =
    ZipArchive::new(cursor).map_err(|e| format!("Failed to open archive: {}", e))?;
...
fn install(source: &str) {
  ...
  if source.starts_with("github:") {
    ...
    let (owner, repo, version) = match parse_github_source(source) {
      ...
      None => {
        eprintln!(
          "✗ Invalid GitHub source format. Use: github:owner/repo or github:owner/repo@version"
        );
        process::exit(1);
      }
    };
    ...
    // Find .rua asset
    let rua_asset = release.assets.iter().find(|a| a.name.ends_with(".rua"));
