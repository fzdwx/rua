  ...
  async function loadProjects(): Promise<ProjectItem[]> {
    ...
    try {
      if (platform === "darwin") {
        ...
        fileContent = await rua.fs.readTextFile("Code/User/globalStorage/storage.json", {
          baseDir: BaseDirectory.AppData,
        });
      } else if (platform === "linux") {
        fileContent = await rua.fs.readTextFile(".config/Code/User/globalStorage/storage.json", {
          baseDir: BaseDirectory.Home,
        });
      } else {
  ...
  rua.on("activate", async () => {
    ...
    const actions: DynamicAction[] = projects.map((project) => {
      ...
      return {
        id: project.id,
        name: `${project.name}`,
        subtitle: `Open ${project.path} use VS Code`,
        section: "",
        badge: "VS Code Recent Projects",
        ...
        keywords: ["code", "vscode", "project", "recent", project.name.toLowerCase()],
