                                ...
                                actions={
                                    <ActionPanel title={todo.title}>
                                        <Action
                                            ...
                                            shortcut={{ key: 'enter' }}
                                            onAction={() => push(<TodoDetail todo={todo} onToggle={toggleTodo} onDelete={deleteTodo} onUpdate={updateTodo} />)}
                                        />
                                        <Action
                ...
                <List.Section title="Active" subtitle={`${activeTodos.length} items`}>
                    {activeTodos.map((todo) => (
                        <List.Item
                            ...
                            keywords={[todo.priority, ...(todo.tags || [])]}
                            accessories={[
                                ...(todo.dueDate ? [{ text: `Due: ${formatDate(todo.dueDate)}`, icon: <Icon source={Icon.Calendar} size={12} /> }] : []),
                                ...
                                <ActionPanel title={todo.title}>
                                    ...
                                    <ActionPanel.Section title="Navigation">
                                        <Action.Push
                                            ...
                                            target={<CreateTodoForm onSubmit={(values) => updateTodo(todo.id, values)} initialValues={todo} isEditing />}
                                        />
                                        <Action
...
`
    ...
    return (
        <Detail
            ...
            metadata={
                <Detail.Metadata>
                    <Detail.Metadata.Label title="Status" text={todo.done ? 'Completed' : 'Active'} icon={<Icon source={todo.done ? Icon.Checkmark : Icon.Clock} />} />
                    <Detail.Metadata.Label title="Priority" text={todo.priority.toUpperCase()} icon={<span>{getPriorityIcon(todo.priority)}</span>} />
                    <Detail.Metadata.Separator />
                    <Detail.Metadata.Label title="Created" text={formatDate(todo.createdAt)} icon={<Icon source={Icon.Calendar} />} />
                    {todo.dueDate && (
                        <Detail.Metadata.Label title="Due Date" text={formatDate(todo.dueDate)} icon={<Icon source={Icon.Clock} />} />
                    )}
            ...
            actions={[
                ...
                },
                {
                    id: 'edit',
                    title: 'Edit',
                    onAction: () => push(<CreateTodoForm onSubmit={(values) => onUpdate(todo.id, values)} initialValues={todo} isEditing />),
                },
                {
                    id: 'delete',
                    title: 'Delete',
