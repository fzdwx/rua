...
export const useIsomorphicLayout = typeof window === "undefined" ? noop : React.useLayoutEffect;
...
export function shouldRejectKeystrokes(
  ...
  const inputs = ["input", "textarea", ...ignoreWhenFocused].map((el) => el.toLowerCase());
  ...
  const ignoreStrokes =
    ...
    (inputs.indexOf(activeElement.tagName.toLowerCase()) !== -1 ||
      ...
      activeElement.attributes.getNamedItem("contenteditable")?.value === "plaintext-only");
